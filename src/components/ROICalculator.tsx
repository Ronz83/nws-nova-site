...
export default function ROICalculator({ onBookDemo }: ROICalculatorProps) {
  ...
  const monthlyDeals = Math.round(leads * (closeRate / 100));
  const newRevenue   = monthlyDeals * dealValue;
  const hoursSaved   = Math.round(leads * 0.75);
  const laborSavings = hoursSaved * 40;
  const totalValue   = newRevenue + laborSavings;
    ...
    <section id="roi" className="py-24 px-6 border-t border-border-base bg-bg-tint relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto flex flex-col gap-12 relative z-10">
        ...
        <div className="text-center max-w-2xl mx-auto">
          <span className="text-xs uppercase tracking-[0.2em] text-sky-600 font-bold">Operational Yield Model</span>
          ...
          <p className="text-xs sm:text-sm text-text-muted mt-3 max-w-md mx-auto leading-relaxed font-medium">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mt-6 items-stretch font-sans">
          ...
          <div className="lg:col-span-6 border-2 border-slate-100 bg-white rounded-[24px] p-8 md:p-9 flex flex-col justify-between gap-8 shadow-sm">
            ...
            <div className="border-t border-slate-100 pt-5 text-[9px] text-text-muted/50 font-mono font-bold text-left leading-relaxed">
          ...
          <div className="lg:col-span-6 grid grid-cols-1 sm:grid-cols-2 gap-5 items-stretch">
            <div className="border-2 border-slate-100 bg-white rounded-[24px] p-6 flex flex-col justify-between text-left relative overflow-hidden shadow-sm hover:shadow-md transition-shadow">
              ...
              <div>
                <span className="text-[9px] uppercase tracking-widest text-text-muted font-bold">Monthly Revenue Yield</span>
                <h3 className="text-3xl font-black tracking-tight mt-1 font-mono text-text-base">${newRevenue.toLocaleString()}</h3>
                <p className="text-[10px] text-text-muted font-medium mt-1.5">Derived from {monthlyDeals} closed deals</p>
              </div>
            </div>
            ...
            <div className="border-2 border-slate-100 bg-white rounded-[24px] p-6 flex flex-col justify-between text-left relative overflow-hidden shadow-sm hover:shadow-md transition-shadow">
              ...
              <div>
                <span className="text-[9px] uppercase tracking-widest text-text-muted font-bold">Saved Labor Hours</span>
                <h3 className="text-3xl font-black tracking-tight mt-1 font-mono text-text-base">{hoursSaved.toLocaleString()}h</h3>
                <p className="text-[10px] text-text-muted font-medium mt-1.5">At ${laborSavings.toLocaleString()} in operational value</p>
              </div>
            </div>
            ...
            <div className="sm:col-span-2 bg-sunrise-gradient rounded-[24px] p-8 flex flex-col justify-between text-left relative overflow-hidden shadow-xl text-white">
              ...
              <div className="flex justify-between items-start mb-6 w-full">
                ...
                <span className="text-[9px] uppercase tracking-widest text-white border border-white/30 bg-white/10 px-3 py-1 rounded-full font-mono font-bold shadow-sm">
                  Total Monthly Yield
                </span>
              </div>
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 w-full">
                <div className="flex-grow">
                  <span className="text-[9px] uppercase tracking-widest text-sky-100 font-bold">Combined Monthly Value</span>
                  <h3 className="text-3xl md:text-4xl font-black tracking-tight mt-0.5 font-mono text-white">${totalValue.toLocaleString()}</h3>
                  <p className="text-[11px] text-sky-100 mt-2 font-medium leading-relaxed">Revenue + operational hours saved, combined.</p>
                </div>
                <button
                  onClick={onBookDemo}
                  className="w-full md:w-auto shrink-0 flex items-center justify-center gap-2 text-xs uppercase tracking-[0.18em] font-bold bg-white hover:bg-slate-50 text-sky-700 px-6 py-4 rounded-xl transition-all duration-200 shadow-md cursor-pointer border-none"
                >
